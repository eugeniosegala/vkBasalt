...
MAKO launches this fork with `VKBASALT_CONFIG_RELOAD=1` and an explicit `VKBASALT_CONFIG_FILE`. The layer checks that file at a bounded interval and updates CAS/DLS strength and DLS denoise through per-swapchain-image uniform buffers. MAKO's FXAA, SMAA, CAS, DLS, Vibrance, Curves, Deband, Technicolor, Sepia, Monochrome, and Vignette selections can switch live. Selection changes drain the layer's graphics queue once before retiring the previous graph, which bounds effect-owned image and pipeline memory instead of retaining every visited combination for the lifetime of the swapchain. FXAA uses its default medium-dither preset and SMAA defaults to its documented medium threshold and search limits; explicit configuration-file values still override the SMAA defaults. Layer activation and unrecognized custom-effect changes remain restart-bound.
