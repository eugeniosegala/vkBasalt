...
MAKO launches this fork with `VKBASALT_CONFIG_RELOAD=1` and an explicit `VKBASALT_CONFIG_FILE`. The layer checks that file at a bounded interval and updates CAS/DLS strength and DLS denoise through per-swapchain-image uniform buffers. It also prepares and retains replacement graphs so MAKO's FXAA, SMAA, CAS, DLS, Vibrance, Curves, and Deband selections can switch live without waiting for the device or destroying in-flight resources. Layer activation and unrecognized custom-effect changes remain restart-bound.
